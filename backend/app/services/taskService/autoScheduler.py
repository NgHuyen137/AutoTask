...
from app.models.taskModel import Task, Split
from app.models.schedulingHourModel import TimeFrame
from app.services.schedulingHourService.crud import get_scheduling_hour_by_id
...
def get_preferred_datetimes(
    ...
    if day_index in list(mapping.keys()): # Check if day is a preferred day 
        for time_frame in mapping[day_index]:
            # The preferred time frame must start from start_date
            if (
                day == start_date.date()
                and start_date.time() > time_frame.start_at 
                and start_date.time() < time_frame.end_at
            ):
                datetimes.append({
                    "start_at": datetime.combine(day, start_date.time(), tzinfo=timezone.utc),
                    "end_at": datetime.combine(day, time_frame.end_at, tzinfo=timezone.utc)
                })
            
            # The preferred time frame is before start_date
            elif (
                (day == start_date.date() and start_date.time() >= time_frame.end_at)
                ...
                datetimes.append({
                    "start_at": datetime.combine(day, time_frame.start_at, tzinfo=timezone.utc),
                    "end_at": datetime.combine(day, time_frame.end_at, tzinfo=timezone.utc)
                })
...
async def fetch_scheduled_blocks(
    id: Optional[str],
    datetimes: List[Dict[str, datetime]], 
    db: AsyncIOMotorDatabase
):
    '''
        Fetch all scheduled blocks that overlap preferred time frames
    '''
    ...
    pipeline = []
    if id:
        pipeline.append({"$match": {"_id": {"$ne": ObjectId(id)}}})
    ...
    results = await db.task_collection.aggregate(pipeline).to_list(length=None)

    # Datetime fetched from MongoDB is naive => Convert naive datetime to UTC timezone  
    scheduled_blocks = []
    if results:
        for doc in results:
            if "scheduled_blocks" in doc:
                for scheduled_block in doc["scheduled_blocks"]:
...
def find_free_slots(scheduled_blocks: List[Dict[str, datetime]], datetimes: List[Dict[str, datetime]]):
    '''
        Find free slots given lists of scheduled blocks and preferred datetimes
    '''
    ...
    for i in range(len(sorted_blocks) - 1):
        if sorted_blocks[i]["end_at"] > sorted_blocks[i+1]["start_at"]:
            sorted_blocks[i]["end_at"] = sorted_blocks[i+1]["end_at"]
            deleted_indices.append(i+1)
    
    if deleted_indices:
        sorted_blocks = [block for i, block in enumerate(sorted_blocks) if i not in deleted_indices]
    
    # Find free time slots
    free_slots = []
    for dt in datetimes:
        # Check if there are scheduled blocks overlap dt
        blocks_overlap_dt = [block for block in sorted_blocks if block["start_at"] < dt["end_at"] and block["end_at"] > dt["start_at"]]
        if not blocks_overlap_dt:
            free_slots.append(dt)
            continue
        
        blocks_overlap_dt.insert(0, {
            "start_at": dt["start_at"],
            "end_at": dt["start_at"]
        })
        
        blocks_overlap_dt.append({
            "start_at": dt["end_at"],
            "end_at": dt["end_at"]
        })
...
async def calculate_free_slots(
    ...
    if datetimes: # The current day is one of the preferred days
        scheduled_blocks = await fetch_scheduled_blocks(id, datetimes, db)
        if scheduled_blocks:
...
def find_split_slots(
    ...
    free_slots = [free_slot for i, free_slot in enumerate(free_slots) if i not in deleted_indices]

    return remaining_duration, time_allocations, free_slots


async def find_optimal_time(task: Task, db: AsyncIOMotorDatabase):
    scheduling_hour = await get_scheduling_hour_by_id(task.scheduling_hour_id, db)
    if scheduling_hour:
        ...
        day = start_date.date() 
        while True:
            free_slots = await calculate_free_slots(task.id, day, start_date, mapping, db)
            if free_slots:
                fitting_slot = find_fitting_slot(converted_duration, free_slots)
                if fitting_slot:
                    if not task.split and fitting_slot["end_at"] > due_date:
                        fitting_slot["is_scheduled_ontime"] = False
                    if fitting_slot["end_at"] <= due_date:
                        fitting_slot["is_scheduled_ontime"] = True
                    if task.split and fitting_slot["end_at"] > due_date:
                        break
